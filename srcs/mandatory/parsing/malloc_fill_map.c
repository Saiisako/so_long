...
t_map	*malloc_map(int fd, char **av, t_game *game)
{
	...
	game->map->grid_length = 0;
	game->map->grid_height = 0;
	line = get_next_line(fd);
	game->map->grid_length = ft_strnlen(line, '\n') - 1;
	while (line)
	{
	...
	free(line);
	game->map->grid = ft_calloc(sizeof(char *), (game->map->grid_height + 1));
	if (!game->map->grid)
		return (NULL);
	close(fd);
	fill_map(game, av, fd);
...
t_game	*tab_dup(t_game *game)
{
	...
	tab = ft_calloc(sizeof(char *), (size_t)(game->map->grid_height + 1));
	if (!tab)
		return (NULL);
	i = 0;
